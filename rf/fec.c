...
static void ant_fec_evt_handler(ant_fec_profile_t * p_profile, ant_fec_evt_t event)
{
	...
	{
		m_fec_info.el_time = ant_fec_utils_raw_time_to_uint16_t(p_profile->page_16.elapsed_time);
		m_fec_info.speed   = ant_fec_utils_raw_speed_to_uint16_t(p_profile->page_16.speed);
//		w_task_events_set(m_tasks_id.boucle_id, TASK_EVENT_FEC_INFO);

		LOG_INFO("#PAGE16: %u %u", m_fec_info.el_time, m_fec_info.speed);
	}
	break;

	case ANT_FEC_PAGE_25_UPDATED:
	{
		m_fec_info.power = p_profile->page_25.inst_power;

		LOG_INFO("#PAGE_25: %u", m_fec_info.power);
	}
	break;
